...
class FixedLeg:
    ...
    def next_payment_date(self):
        frequency = str(self.payment_frequency.upper()).strip().upper()
        if frequency in ["03M", "3M"]:
            payment_date = self.dated_date + relativedelta(months=3)
            return payment_date
        elif frequency in ["06M", "6M"]:
            payment_date = self.dated_date + relativedelta(months=6)
            return payment_date
        elif frequency in ["12M", "012M"]:
    ...
    def __str__(self):
        return (
            ...
            f"\tAccounting basis: {self.accounting_basis_month.upper()}/{self.accounting_basis_year.upper()}, "
            f"Dated date: {self.dated_date}, "
            f"Accrued interest: {self.accrued_interest()} {self.currency.upper()}, "
            f"Next payment date: {self.next_payment_date()}")
...
class Swap:
    ...
    def trade(self, direction: str, amount):
        if direction.upper() == "BUY":
            ...
            print(f"\nPosition {self.swap_id} has been increased successfully by {amount}")
            ...
            if self.receivable_leg.notional <= 0 or self.payable_leg.notional <= 0:
                ...
                print(f"\nPosition {self.swap_id} has been fully closed\n")
            else:
                print(f"\nPosition {self.swap_id} has been decreased successfully by {amount}")
        else:
...
class Portfolio:
    ...
    def add_swap(self, position):
        ...
        print(f"Position {position.swap_id} added successfully.")
    ...
    def remove_closed_positions(self):
        ...
        self.positions = [position for position in self.positions if position.receivable_leg.notional > 0]
        removed_count = initial_count - len(self.positions)
        print(f"Closed positions removed successfully. Removed: {removed_count}")
